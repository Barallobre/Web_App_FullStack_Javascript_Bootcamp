...
async function agregarEquipo(IdEspacio,IdTipoEquipamiento,Cantidad){
  await espaciosRepository.insertEquipamiento(IdEspacio,IdTipoEquipamiento,Cantidad);
}
...
//función para guardar una imagen subida desde el ordenador al servidor y redimensionarla
function insertarFoto(imagen,nombreImagen) {
  let picture;
  ...
  setTimeout(function(){ 
    ...
    Jimp.read(imagen, (err, img) => {
      ...
      img
        .resize(250, 150) 
        .write(__dirname+'/../files/espacios/'+myImage+'.png'); 
    });
  }
  console.log('Foto subida a: '+__dirname+'/../files/espacios/'+myImage+'.png')
  return picture = __dirname+'/../files/espacios/'+myImage+'.png';
}
...
async function addEspacio(req,res){
  try {
    ...
    await schema.validateAsync({nombre,localidad,tipoEspacio,fechaInicioDisp,fechaFinDisp,costeDia});
    ...
    if(req.files !== null){
      ...
      picture = insertarFoto(imagen,nombreImagen);
    }
    await espaciosRepository.addEspacio(nombre,localidad,tipoEspacio,fechaInicioDisp,fechaFinDisp,costeDia,picture);
    
    //buscamos el Id del espacio nuevo para insertar en él el equipamiento que se encuentra registrado en otra tabla
    const IdEspacio = await espaciosRepository.buscarIdEspacio(nombre,tipoEspacio);
    ...
    let Cantidad;
    const equipamiento = [sillas,mesas,proyector, pantallaProyector,monitores];
    const equipamiento_2= ["sillas","mesas","proyector", "pantallaProyector","monitores"];
    for(let i = 0;i<equipamiento.length;i++){
      if(equipamiento[i]!==undefined){
        const IdTipoEquipamiento = await espaciosRepository.buscarTipoEquipamiento(equipamiento_2[i]);
...
async function updateEspacio(req,res){
  try {
    ...
    let { nombre,localidad,tipoEspacio,fechaInicioDisp,fechaFinDisp,costeDia } = req.body;
