...
async function listEspacios(){
  ...
  const query = 'select Espacio.IdEspacio,Espacio.Nombre, TipoEspacio.NombreTipo "Tipo de espacio" from Espacio join TipoEspacio on Espacio.idTipoEspacio = TipoEspacio.IdTipoEspacio';
...
async function addEspacio(nombre,localidad,idTipoEspacio,fechaInicioDisp,fechaFinDisp,costeDia,picture){
  const pool = await database.getPool();  
  const query = 'INSERT INTO Espacio (Nombre,Localidad, idTipoEspacio,FechaInicioDisponibilidad,FechaFinDisponibilidad,CosteDiario,Foto) VALUES ( ? , ? , ? , ? , ? , ? , ?)';
  await pool.query(query, [nombre,localidad,idTipoEspacio,fechaInicioDisp,fechaFinDisp,costeDia,picture]);
  return true;
}
async function buscarIdEspacio(nombre,idTipoEspacio){
  const pool = await database.getPool(); 
  const query = 'SELECT IdEspacio FROM Espacio WHERE Nombre = ? and idTipoEspacio = ?';
...
async function deleteEquipamiento(IdEspacio,IdTipoEquipamiento){
  const pool = await database.getPool();  
  const deleteQuery = 'DELETE FROM EquipamientoEspacio WHERE IdEspacio = ? and IdTipoEquipamiento = ?';
