...
async function createIncidencia(
  ...
  const insertQuery =
    "INSERT INTO Incidencia (idTipoIncidencia, idEspacio, Descripcion, FechaAlta) VALUES (?, ?, ?, ?)";
...
async function getIncidencias(id) {
  const pool = await database.getPool();
  const query =
    'SELECT Incidencia.IdIncidencia, TipoIncidencia.NombreTipo, DATE_FORMAT(Incidencia.FechaAlta, "%d/%m/%Y") as "FechaAlta", Incidencia.Descripcion from Incidencia join TipoIncidencia on Incidencia.idTipoIncidencia = TipoIncidencia.IdTipoIncidencia WHERE idEspacio = ? ';
  const [incidencia] = await pool.query(query, id);
  return incidencia;
}
